...
fn create_test_parquet_file(num_rows: usize, temp_dir: &TempDir) -> std::path::PathBuf {
    ...
    let labels: Vec<String> = (1..=num_rows)
        .map(|i| if i % 2 == 0 { "Person" } else { "Company" })
        .map(|s| s.to_string())
        .collect();
...
fn bulk_load_complete_graph_benchmark(c: &mut Criterion) {
    ...
    for size in [1000, 10000].iter() {
        ...
        let types: Vec<String> = (1..=*size)
            .map(|i| if i % 2 == 0 { "KNOWS" } else { "WORKS_FOR" })
            .map(|s| s.to_string())
            .collect();
        ...
        let props = WriterProperties::builder().build();
        let mut writer =
            ArrowWriter::try_new(rels_file, std::sync::Arc::new(rels_schema), Some(props)).unwrap();
...
fn bulk_load_nodes_with_deduplication_benchmark(c: &mut Criterion) {
    ...
    for size in [1000, 10000, 100000].iter() {
        ...
        let emails: Vec<String> = (1..=*size)
            .map(|i| format!("user{}@example.com", i / 10))
            .collect();
        ...
        let props = WriterProperties::builder().build();
        let mut writer =
            ArrowWriter::try_new(file, std::sync::Arc::new(schema), Some(props)).unwrap();
        writer.write(&batch).unwrap();
        writer.close().unwrap();
...
fn bulk_load_relationships_with_deduplication_benchmark(c: &mut Criterion) {
    ...
    for size in [1000, 10000, 100000].iter() {
        ...
        let size_val = *size;
        let from_ids: Vec<i64> = (0..size_val)
            .map(|i| (i % (size_val / 10).max(1)) as i64 + 1)
            .collect();
        let to_ids: Vec<i64> = (0..size_val)
            .map(|i| ((i + 1) % (size_val / 10).max(1)) as i64 + 1)
            .collect();
            ...
            b.iter(|| {
                ...
                for i in 0..size {
                    ...
                    builder
                        ...
                        .unwrap();
                    builder
                        .set_prop_bool(i as u32, "active", i % 2 == 0)
                        .unwrap();
                }
                black_box(builder);
            });
...
fn parquet_vs_regular_with_dedup_benchmark(c: &mut Criterion) {
    ...
    for size in [10000, 100000].iter() {
        ...
        group.bench_with_input(BenchmarkId::new("parquet_dedup", *size), size, |b, _| {
            b.iter(|| {
                let mut builder = GraphBuilder::new(Some(*size), Some(0));
                builder
                    .load_nodes_from_parquet(
...
fn finalize_with_deduplication_benchmark(c: &mut Criterion) {
    ...
    for size in [10000, 100000].iter() {
        ...
        for i in 0..*size {
            ...
            setup_builder
                .set_prop_str(i as u32, "email", &email)
                .unwrap();
        }
        for i in 0..*size {
        ...
        group.bench_with_input(BenchmarkId::new("with_dedup", *size), size, |b, &size| {
            b.iter(|| {
                let mut builder = GraphBuilder::new(Some(size), Some(size));
                builder.enable_node_deduplication(vec!["email"]);
                for i in 0..size {
