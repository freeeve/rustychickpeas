...
fn get_ldbc_data_dir() -> PathBuf {
    ...
    if !sf_path.exists() {
        ...
        for fallback_sf in fallbacks {
            if fallback_sf != scale_factor {
                ...
                if fallback_path.exists() {
                    println!(
                        "SF{} not found, using SF{} as fallback",
                        scale_factor, fallback_sf
                    );
        ...
        println!(
            "Warning: No LDBC data found. Tried SF{}, SF0.003, SF1, and SF10",
            scale_factor
        );
    }
                ...
                if let Ok(mut reader) = builder_reader.build() {
                    while let Some(Ok(batch)) = reader.next() {
                        if let Some(id_array) =
                            batch.column(id_idx).as_any().downcast_ref::<Int64Array>()
                        {
...
fn load_relationships_with_mapping(
    ...
    for batch_result in reader {
        ...
        if let Some(start_array) = start_col_data.as_any().downcast_ref::<Int64Array>() {
            if let Some(end_array) = end_col_data.as_any().downcast_ref::<Int64Array>() {
                for i in 0..batch.num_rows() {
                    ...
                    {
                        builder
                            .add_relationship(start_internal, end_internal, rel_type)
                            .unwrap();
                        count += 1;
                    }
                }
        ...
        if batch_num % 10 == 0 || count % 1_000_000 == 0 {
            ...
            println!(
                "    Batch {}: {} relationships loaded ({:.0} rels/sec)",
                batch_num, count, rate
            );
        }
    }
    ...
    let rate = count as f64 / elapsed.as_secs_f64();
    println!(
        "    ✓ Loaded {} relationships in {:.2}s ({:.0} rels/sec)",
        count,
        elapsed.as_secs_f64(),
        rate
    );
...
fn load_ldbc_graph() -> GraphSnapshot {
    ...
    println!(
        "Creating GraphBuilder with capacity: {} nodes, {} relationships",
        node_capacity
            .map(|n| n.to_string())
    ...
    let mut ldbc_to_internal: std::collections::HashMap<i64, u32> =
    ...
    let person_files: Vec<_> = if let Ok(entries) = std::fs::read_dir(dynamic_dir.join("Person")) {
        ...
        let fallback = dynamic_dir
            .join("Person")
            .join("part-00000-0e4e6296-69d1-410b-ae31-b48e15ad8ff8-c000.snappy.parquet");
        if fallback.exists() {
            vec![fallback]
        } else {
            vec![]
    ...
    if person_files.is_empty() {
        eprintln!(
            "  No Person files found in {}",
            dynamic_dir.join("Person").display()
        );
    } else {
        for person_file in person_files {
            if person_file.exists() {
                println!(
                    "  Reading Person parquet file: {}",
                ...
                match builder.load_nodes_from_parquet(
                    ...
                    Ok(node_ids) => {
                        ...
                        println!(
                            "  ✓ Loaded {} Person nodes from {} in {:.2}s ({:.0} nodes/sec)",
                            node_ids.len(),
                            person_file.file_name().unwrap().to_string_lossy(),
                        ...
                        println!(
                            "  ✓ Built mapping in {:.2}s ({:.0} nodes/sec)",
                    ...
                    Err(e) => {
                        eprintln!(
                            "  Error loading Person nodes from {}: {}",
                            person_file.display(),
                            e
                        );
                    }
    ...
    let forum_files: Vec<_> = if let Ok(entries) = std::fs::read_dir(dynamic_dir.join("Forum")) {
        ...
        let fallback = dynamic_dir
            .join("Forum")
            .join("part-00000-f9d0166d-0401-4a20-aa0d-413ca642769a-c000.snappy.parquet");
        if fallback.exists() {
            vec![fallback]
        } else {
            vec![]
    ...
    println!(
        "  Forum loading complete in {:.2}s",
        forum_start.elapsed().as_secs_f64()
    );
    ...
    let post_files: Vec<_> = if let Ok(entries) = std::fs::read_dir(dynamic_dir.join("Post")) {
        ...
        let fallback = dynamic_dir
            .join("Post")
            .join("part-00000-5bdef25d-efac-42a3-8eba-944f1e2c3c41-c000.snappy.parquet");
        if fallback.exists() {
            vec![fallback]
        } else {
            vec![]
    ...
    let comment_files: Vec<_> = if let Ok(entries) = std::fs::read_dir(dynamic_dir.join("Comment"))
    {
        ...
        let fallback = dynamic_dir
            .join("Comment")
            .join("part-00000-af764493-8c73-4cf8-a03f-c0024a693cec-c000.snappy.parquet");
        if fallback.exists() {
            vec![fallback]
        } else {
            vec![]
    ...
    println!(
        "  Comment loading complete in {:.2}s",
        comment_start.elapsed().as_secs_f64()
    );
    ...
    let knows_files: Vec<_> =
        if let Ok(entries) = std::fs::read_dir(dynamic_dir.join("Person_knows_Person")) {
            entries
                .flatten()
                .filter(|e| e.path().extension().and_then(|s| s.to_str()) == Some("parquet"))
                .map(|e| e.path())
            ...
            let fallback = dynamic_dir
                ...
                .join("part-00000-dec690ec-1d14-4dde-8a58-103439838a42-c000.snappy.parquet");
            if fallback.exists() {
                vec![fallback]
            } else {
                vec![]
    ...
    for (file_idx, knows_file) in knows_files.iter().enumerate() {
        println!(
            ...
            file_idx + 1,
            knows_files.len(),
            knows_file.file_name().unwrap().to_string_lossy()
        );
    ...
    println!(
        "  ✓ Loaded {} Person_knows_Person relationships in {:.2}s ({:.0} rels/sec)",
        total_knows,
        knows_time.as_secs_f64(),
        total_knows as f64 / knows_time.as_secs_f64()
    );

    println!("Loading Person_hasInterest_Tag relationships...");
    let has_interest_files: Vec<_> =
        if let Ok(entries) = std::fs::read_dir(dynamic_dir.join("Person_hasInterest_Tag")) {
            entries
                .flatten()
                .filter(|e| e.path().extension().and_then(|s| s.to_str()) == Some("parquet"))
                .map(|e| e.path())
            ...
            let fallback = dynamic_dir
                ...
                .join("part-00000-39b5c7e5-9f06-4fac-9d36-a2b1931bcf9f-c000.snappy.parquet");
            if fallback.exists() {
                vec![fallback]
            } else {
                vec![]
    ...
    let likes_post_file = dynamic_dir
        .join("Person_likes_Post")
        .join("part-00000-d69b501e-4045-4df4-a5db-2076aaa3a995-c000.snappy.parquet");
    ...
    let likes_comment_file = dynamic_dir
        .join("Person_likes_Comment")
        .join("part-00000-2a84e34f-f6a6-4541-a864-7e4b30b346e0-c000.snappy.parquet");
    ...
    let has_member_file = dynamic_dir
        .join("Forum_hasMember_Person")
        .join("part-00000-ddeabbe4-5f94-40de-bfc8-929a216331da-c000.snappy.parquet");
    ...
    let post_has_tag_file = dynamic_dir
        .join("Post_hasTag_Tag")
        .join("part-00000-bf665605-8a1f-4037-a607-cc8a6007b563-c000.snappy.parquet");
    ...
    let comment_has_tag_file = dynamic_dir
        .join("Comment_hasTag_Tag")
        .join("part-00000-a4188313-607c-4b07-bdb0-8c10d9dd349c-c000.snappy.parquet");
    ...
    let forum_has_tag_file = dynamic_dir
        .join("Forum_hasTag_Tag")
        .join("part-00000-059e15c0-7560-466f-80ec-a5ed55d9f184-c000.snappy.parquet");
    ...
    println!("✓ Graph finalized in {:.2}s", finalize_time.as_secs_f64());
    println!(
...
fn bi1_tag_evolution() {
    ...
    for &post_id in &posts {
        let post_tags = graph
            .neighbors_by_type(post_id, Direction::Outgoing, &["hasTag"])
            .collect::<Vec<_>>();
    ...
    for &comment_id in &comments {
        let comment_tags = graph
            .neighbors_by_type(comment_id, Direction::Outgoing, &["hasTag"])
            .collect::<Vec<_>>();
    ...
    assert!(
        !tag_pairs.is_empty() || posts.is_empty(),
        "BI1 should find some tag pairs or have no posts"
    );
}
...
fn bi2_tag_person_path() {
    ...
    for &person1_id in persons.iter().take(sample_size) {
        ...
        let person1_tags = graph
            .neighbors_by_type(person1_id, Direction::Outgoing, &["hasInterest"])
            .collect::<Vec<_>>();
    ...
    let elapsed = start.elapsed();
    println!(
        "BI2: Found {} tag-person paths in {:.3}ms",
        paths,
        elapsed.as_secs_f64() * 1000.0
    );
    assert!(
        paths > 0 || persons.is_empty(),
        "BI2 should find paths or have no persons"
    );
}
...
fn bi3_popular_topics() {
    ...
    let elapsed = start.elapsed();
    println!(
        "BI3: Found {} popular tags (top: {:?}) in {:.3}ms",
        top_tags.len(),
...
fn bi4_top_commenters() {
    ...
    if let Some(comments) = graph.nodes_with_label("Comment") {
        for comment_id in comments.iter() {
            ...
            let creators = graph
                .neighbors(comment_id, Direction::Incoming)
                .collect::<Vec<_>>();
    ...
    let elapsed = start.elapsed();
    println!(
        "BI4: Found {} top commenters in {:.3}ms",
        top_commenters.len(),
        elapsed.as_secs_f64() * 1000.0
    );
}
...
fn bi5_active_users() {
    ...
    if let Some(posts) = graph.nodes_with_label("Post") {
        for post_id in posts.iter() {
            // Find creator
            let creators = graph
                .neighbors(post_id, Direction::Incoming)
                .collect::<Vec<_>>();
    ...
    let elapsed = start.elapsed();
    println!(
        "BI5: Found {} active users in {:.3}ms",
        top_users.len(),
        elapsed.as_secs_f64() * 1000.0
    );
}
...
fn bi6_tag_cooccurrence() {
    ...
    let elapsed = start.elapsed();
    println!(
        "BI6: Found {} tag co-occurrence pairs in {:.3}ms",
        top_pairs.len(),
        elapsed.as_secs_f64() * 1000.0
    );
}
...
fn run_all_bi_queries() {
    ...
    println!(
        "BI6 completed in {:.3}ms\n",
        query_start.elapsed().as_secs_f64() * 1000.0
    );

    let total_elapsed = total_start.elapsed();
    println!(
        "=== Total Benchmark Time: {:.3}s ===",
