...
- `snapshot.shortest_path(source, target, rel_types=None, direction=Both,
  ...
  rel property (via `relationship_property`) as the rel weight; otherwise
  every followed rel costs 1.0 (hop count). Returns `None` if unreachable.
...
unweighted hop-count, and weighted (a graph where the cheap rel differs
from the fewest-hops rel).
