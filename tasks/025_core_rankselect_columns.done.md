...
rel columns. Add a rank/select `Column` variant for the moderately-sparse band
...
- **Timings (same load window):**
  ...
  - BI: no regression (noise-level; rel-prop readers Q11 4.0→3.3, Q19 6.4→5.8 a bit faster).

### Extension (f64/bool/str + node columns)

- Added `RankF64`/`RankBool`/`RankStr` variants; routed both rel and node column
...
Its only rel is ~1.6× less memory than sparse — a modest absolute saving
