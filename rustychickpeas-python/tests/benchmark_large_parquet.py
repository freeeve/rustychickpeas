...
def _print_batch_progress(batch_index, processed, total, start, unit):
    """Print loading throughput every 100th batch as a carriage-return progress line."""
...
def _write_large_nodes_file(nodes_file, num_nodes, batch_size, start):
    """Write the nodes Parquet file in batches to bound memory usage."""
    schema = pa.schema([
...
def _write_large_rels_file(rels_file, num_relationships, num_nodes, batch_size, start):
    """Write the relationships Parquet file in batches to bound memory usage."""
...
def create_large_parquet_files(num_nodes=25_000_000, num_relationships=25_000_000):
    ...
    print(f"\n{'='*70}")
    print("Creating Large Parquet Files")
    print(f"{'='*70}")
    ...
    _write_large_nodes_file(nodes_file, num_nodes, batch_size, start)
    ...
    _write_large_rels_file(rels_file, num_relationships, num_nodes, batch_size, start)
...
def benchmark_large_load(nodes_file, rels_file):
    ...
    print(f"\n{'='*70}")
    print("Benchmarking Large Parquet Load (Streaming)")
    print(f"{'='*70}\n")
...
def main():
    ...
    try:
        benchmark_large_load(nodes_file, rels_file)
